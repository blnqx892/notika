...
if ($bandera=="GuardarEquipo") {
    $nombre=$_POST["nombre"];
    $categoria=$_POST["categoria"];
    $stock=$_POST["stock"];
    $caracte=$_POST["caracte"];
    $sql="INSERT INTO producto (nombre_Pro, categoria_Pro, stock_Pro, caracteristicas, distinto, tipo_Prod) VALUES 
('$nombre', '$categoria', '$stock', '$caracte', 0, 1)";

    mysqli_query($conexion, $sql) or die ("Error no conectai".mysqli_connect_error());
    echo"
<script language='javascript'>alert('Registro de Equipo Exitoso!!')
 window.location='/Funesi/notika/green-horizotal/ListaEquipo.php'
    </script>";
 //////////CAPTURA DATOS PARA BITACORA
    $usuari=$_SESSION['usuarioActivo']['usuario'];
    $sql="INSERT INTO bitacora (usuario_Usu,sesionInicio,actividad) VALUES ('$usuari',now(),'Registró un equipo')";
